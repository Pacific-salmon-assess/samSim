...
genericRecoverySim <- function(simPar, cuPar, catchDat=NULL, srDat=NULL,
  ...
  HCRmetric <- ifelse(is.null(simPar$HCRmetric), singleHCR, simPar$HCRmetric) #metric used for abundance-based management retrospective or forecast
  if(singleHCR %in% c("forecast","retro")|is.null(simPar$HCRmetric)){warning("forecast/retro neeeds to be defined in simPar$HCRmetric, simPar$singleHCR should take TRUE/FALSE values only")}
  ...
  rampER <- ifelse(is.null(simPar$rampER),FALSE,simPar$rampER)
  ...
  totalCACatchArray <- array(NA, dim = c(nYears, nCU, nTrials))
  totalUSCatchArray <- array(NA, dim = c(nYears, nCU, nTrials))
  ...
  for (n in 1:nTrials) {
   ...
   trendCanER.iter<-matrix(nrow=nYears,ncol = nCU)
    ...
    for (y in 1:nPrime) {
          ...
          for (k in 1:nCU) {
            ...
            if (model[k] == "ricker") {
              ...
              sMSY[y, k, n] <- (1 - gsl::lambert_W0(exp(1 - refAlpha[k]))) /
                beta[k]
              sGen[y, k, n] <- as.numeric(max(samEst::sgenCalcDirect(refAlpha[k], beta[k]),0))
            ...
            if (model[k] == "rickerSurv") {
              ...
              sMSY[y, k, n] <- (1 - gsl::lambert_W0(exp(1 - refAlpha_prime))) /
                beta[k]
              sGen[y, k, n] <- as.numeric(max(samEst::sgenCalcDirect(refAlpha_prime, beta[k]),0))
          ...
          for (k in 1:nCU) {
            if (model[k] == "ricker" | model[k] == "rickerSurv" |
              ...
              if (bm == "habitat"){
                ...
                sGen_habitat[y, k, n] <- as.numeric(max(samEst::sgenCalcDirect(refAlpha[k], beta[k]),0))
      ...
      bmUMSY[y, , n] <- estUMSY[y, , n]#rep(canER,nCU) #initial benchmark (before nPrime based on start ER)
  ...
  #                                      spwnMat = S)
    ...
    for (y in (nPrime + 1):nYears) {
      ...
      for (k in 1:nCU) {
        if (model[k] == "ricker") {
          if (normPeriod == TRUE) {
            ...
            sGen[y, k, n] <- as.numeric(max(samEst::sgenCalcDirect(alphaMat[y,k], betaMat[y,k]),0))
      ...
      for (k in 1:nCU) {
        ...
        #fixed ER target, independent of run size, note it doesn't matter if forecast or retro management is used
        if(HCRtype=='fixed'){
          
          trendCanER.iter[y,k]<-trendCanER[y,k]
        
        }else if(HCRtype=='umsy'){ #sets ER based on last umsy benchmark at assessment times the er adjustment
          
          #estimated UMSY-based target, independent of run size,note it doesn't matter if forecast or retro management is used
          trendCanER.iter[y,k] <- bmUMSY[y-1,k,n]
        
        }else if(HCRtype=='abundance'){ #for HCRs with fixer exploitation rates for each abundance zone (independent of estimates UMSY)
          
          if(is.na(redStatusER)){
            tmpredStatusER<-max(trendCanER[y,k]*bmERAdj,minER,na.rm=T)
          }else{
            tmpredStatusER <- redStatusER
          }
          
          if(is.na(amberStatusER)){
            tmpamberStatusER<-max(trendCanER[y,k]*bmERAdj,minER,na.rm=T)
          }else{
          ...
          if(HCRmetric=="retro"){ #retrospective branch 
            
            if(counterLowerObsBM[y-1, k]==0&counterUpperObsBM[y-1, k]==0){
              #red status
              trendCanER.iter[y,k]<- tmpredStatusER           
            }else if(counterLowerObsBM[y-1, k]==1&counterUpperObsBM[y-1, k]==0&!rampER){#amber status constant ER
              #amber status
              trendCanER.iter[y,k]<- tmpamberStatusER
            }else if(counterLowerObsBM[y-1, k]==1&counterUpperObsBM[y-1, k]==0&rampER){#amber status ramp ER
              #use redStatusER instead oftemp to ensure thatramp goes to zero (minER) if desired
              if(!is.na(redStatusER)){
                ER.temp=redStatusER+(trendCanER[y,k]-redStatusER)*((obsS[y-1, k]-lowerObsBM[y-1,k])/(upperObsBM[y-1,k]-lowerObsBM[y-1,k]))
            ...
            if((foreRecRY[y, k]*(1-trendCanER[y,k]))>=upperObsBM[y-1,k]){
              #green status
              trendCanER.iter[y,k]<- trendCanER[y,k]

            }else if(foreRecRY[y, k]*(1-redStatusER)<=lowerObsBM[y-1,k]){
              #red status
              trendCanER.iter[y,k]<- tmpredStatusER              
            }else{
              #amber status
              if(rampER){
                ...
                if(foreRecRY[y, k]*(1-tmpamberStatusER)<=lowerObsBM[y-1,k]){
                  ...
                  trendCanER.iter[y,k] <- max(tmpamberStatusER,(foreRecRY[y, k]-upperObsBM[y-1,k])/foreRecRY[y, k])
                }
              }
            }
          }     
        }else if(HCRtype=='both'){ #sets ER based on last umsy benchmark at assessment times the er adjustment
          
          trendCanER.iter[y,k] <- bmUMSY[y-1,k,n]
          if(is.na(redStatusER)){
            tmpredStatusER<-max(trendCanER.iter[y,k]*bmERAdj,minER,na.rm=T)
          }else{
            tmpredStatusER <- redStatusER
          }
          
          if(is.na(amberStatusER)){
            tmpamberStatusER<-max(trendCanER.iter[y,k]*bmERAdj,minER,na.rm=T)
          }else{
            tmpamberStatusER <- amberStatusER
          }
          ...
          if(HCRmetric=="retro"){ 
            ...
            if(counterLowerObsBM[y-1, k]==0&counterUpperObsBM[y-1, k]==0){
              ...
              if(!is.na(redStatusER)){
                ER.temp=redStatusER+(trendCanER.iter[y,k]-redStatusER)*((obsS[y-1, k]-lowerObsBM[y-1,k])/(upperObsBM[y-1,k]-lowerObsBM[y-1,k]))
                #scale trandCanER by the benchmark adjustment and a ratio of the run forecast excess (above lower Obs BM) vs. the upper/lower obs BM
              }else{
            ...
            }else if(foreRecRY[y, k]*(1-tmpredStatusER)<=lowerObsBM[y-1,k]){
              #red status
              trendCanER.iter[y,k]<- tmpredStatusER              
            }else{
              #amber status
   ...
   #old code for reference
    ...
    ##      if(HCRtype=='abundance'){ #for HCRs with fixer exploitation rates for each abundance zone (independent of estimates UMSY)
    ##        if(counterLowerObsBM[y-1, k]==0&counterUpperObsBM[y-1, k]==0){
    ##          #red status
    ##          if(!is.na(redStatusER)){
    ...
    ##            ER.temp=redStatusER+trendCanER[y,k]*((obsS[y-1, k]-lowerObsBM[y-1,k])/(upperObsBM[y-1,k]-lowerObsBM[y-1,k]))
    ##            #scale trandCanER by the benchmark adjustment and a ratio of the run forecast excess (above lower Obs BM) vs. the upper/lower obs BM
    ##          }else{
    ##            ER.temp=trendCanER[y,k]*((obsS[y-1, k]-lowerObsBM[y-1,k])/(upperObsBM[y-1,k]-lowerObsBM[y-1,k]))
    ##          }
...
##
    ...
    ##        else if(counterLowerObsBM[y-1, k]==1&counterUpperObsBM[y-1, k]==0&rampER){
    ##          if(!is.na(redStatusER)==F){
    ##            ER.temp=redStatusER+trendCanER.iter[y,k]*((obsS[y-1, k]-lowerObsBM[y-1,k])/(upperObsBM[y-1,k]-lowerObsBM[y-1,k]))
    ##            #scale trandCanER by the benchmark adjustment and a ratio of the run forecast excess (above lower Obs BM) vs. the upper/lower obs BM
    ##          }else{
    ##            ER.temp=trendCanER.iter[y,k]*((obsS[y-1, k]-lowerObsBM[y-1,k])/(upperObsBM[y-1,k]-lowerObsBM[y-1,k]))
    ##          }
    ...
    ##          trendCanER.iter[y,k] <- redStatusER
    ##         
    ##        }else if((foreRecRY[y, k]*(1-trendCanER[y,k])>=lowerObsBM[y-1,k]&
    ##                  foreRecRY[y, k]*(1-trendCanER[y,k])<upperObsBM[y-1,k])|
    ##                 (foreRecRY[y, k]*(1-amberStatusER)>=lowerObsBM[y-1,k]&
    ##                  foreRecRY[y, k]*(1-amberStatusER)<upperObsBM[y-1,k])&
    ##          !rampER){
    ##          #amber status
    ##          trendCanER.iter[y,k]<- amberStatusER
    ##          
    ##        }else if((foreRecRY[y, k]*(1-trendCanER[y,k]))>lowerObsBM[y-1,k]&
    ##          (foreRecRY[y, k]*(1-trendCanER[y,k]))<upperObsBM[y-1,k]&rampER){
    ##          #scale trandCanER by the benchmark adjustment and a ratio of the run forecast excess (above lower Obs BM) vs. the upper/lower obs BM
    ##          if(!is.na(redStatusER)==F){
    ##            ER.temp=redStatusER+trendCanER[y,k]*(((foreRecRY[y, k]*(1-trendCanER[y,k]))-lowerObsBM[y-1,k])/(upperObsBM[y-1,k]-lowerObsBM[y-1,k]))
    ##          }else{
    ...
    ##      if(HCRtype=='both'){ #sets ER based on last umsy benchmark at assessment times the er adjustment
    ##        trendCanER.iter[y,k] <- bmUMSY[y-1,k,n]
    ##        if((foreRecRY[y, k]*(1-trendCanER[y,k]))<lowerObsBM[y-1,k]){
    ##          #red status
    ##          if(is.na(redStatusER)==F){
    ##            trendCanER.iter[y,k]<- redStatusER
    ##          }else{
    ...
    ##          if(!is.na(redStatusER)==F){
    ##            ER.temp=redStatusER+trendCanER.iter[y,k]*(((foreRecRY[y, k]*(1-trendCanER[y,k]))-lowerObsBM[y-1,k])/(upperObsBM[y-1,k]-lowerObsBM[y-1,k]))
    ##            #scale trandCanER by the benchmark adjustment and a ratio of the run forecast excess (above lower Obs BM) vs. the upper/lower obs BM
    ##          }else{
  ...
  if(aggDatOut==TRUE){
    ...
    aggDat <- data.frame(opMod = rep(nameOM, length.out = nTrials),
                       ...
                       targetER = apply(matrix(targetExpRateAg[(nPrime + 1):nYears, ]), 2, median), #median target ER (stable through time unless TAM rule active)
                       medSpawners = apply(matrix(sAg[yrsSeq, ]), 2, median), #median aggregate spawner abundance across management period (i.e. loop 3 when status is assessed)
                       varSpawners = apply(matrix(sAg[yrsSeq, ]), 2, cv), #cv aggregate spawner abundance across management period
                       medObsSpawners = apply(matrix(obsSAg[yrsSeq, ]), 2, median), #median aggregate estimated spawner abundance across management period (i.e. loop 3 when status is assessed)
                       varObsSpawners = apply(matrix(obsSAg[yrsSeq, ]), 2, cv), #cv aggregate estimated spawner abundance across management period
                       medRecRY = apply(matrix(recRYAg[yrsSeq, ]), 2, median), #median aggregate spawner abundance across management period (i.e. loop 3 when status is assessed)
                       varRecRY = apply(matrix(recRYAg[yrsSeq, ]), 2, cv), #cv aggregate recruit abundance across management period
                       medRecBY = apply(matrix(recBYAg[yrsSeq, ]), 2, median), #median aggregate recruit abundance across management period (i.e. loop 3 when status is assessed)
                       varRecBY = apply(matrix(recBYAg[yrsSeq, ]), 2, cv), #cv aggregate recruit abundance across management period
                       medObsRecBY = apply(matrix(obsRecBYAg[yrsSeq, ]), 2,
                       ...
                       varObsRecBY = apply(matrix(obsRecBYAg[yrsSeq, ]), 2, cv), #cv aggregate estimated recruit abundance across management period
                       medObsRecRY = apply(matrix(obsRecRYAg[yrsSeq, ]), 2, median), #median aggregate estimated recruit abundance across management period (i.e. loop 3 when status is assessed)
                       varObsRecRY = apply(matrix(obsRecRYAg[yrsSeq, ]), 2, cv), #cv aggregate estimated recruit abundance across management period
                       medSpawnersLate = apply(matrix(sAg[(nYears - (3 * gen)):nYears, ]), 2, median), #median aggregate spawner abundance in last 2 generations of management period
                       medCatch = apply(matrix(catchAg[yrsSeq, ]), 2, median), #median aggregate catch across management period
                       varCatch = apply(matrix(catchAg[yrsSeq, ]), 2, cv), #cv aggregate catch across management period
                       stabilityCatch = apply(matrix(catchAg[yrsSeq, ]), 2,
                                              function(x) 1 / cv(x)), #stability of aggregate catch across management period
                       medObsCatch = apply(matrix(obsCatchAg[yrsSeq, ]), 2, median), #median aggregate catch across management period
                       varObsCatch = apply(matrix(obsCatchAg[yrsSeq, ]), 2, cv), #cv aggregate catch across management period
                       stabilityObsCatch = apply(matrix(obsCatchAg[yrsSeq, ]), 2,
                                                 function(x) 1 / cv(x)), #stability of obs aggregate catch across management period
                       medCatchLate = apply(matrix(catchAg[(nYears - 3*gen):nYears, ]), 2, median), #median aggregate catch in last 2 generations of management period
                       medER = apply(matrix(expRateAg[yrsSeq,]), 2, median), #median true aggregate ER
                       medObsER = apply(matrix(obsExpRateAg[yrsSeq, ]), 2, median), #median true aggregate ER
  ...
  for(i in 1:nTrials) {
    ...
    counterLowerBM.i<-as.data.frame(counterLowerBMArray[,,i])
    counterUpperBM.i<-as.data.frame(counterUpperBMArray[,,i])
    counterLowerObsBM.i<-as.data.frame(counterLowerObsBMArray[,,i])
    counterUpperObsBM.i<-as.data.frame(counterUpperObsBMArray[,,i])
    totalCatch.i<-as.data.frame(totalCatchArray[,,i])
    totalCACatch.i<-as.data.frame(totalCACatchArray[,,i])
    totalUSCatch.i<-as.data.frame(totalUSCatchArray[,,i])
    ...
    if(nCU==1){
      ...
      names(counterLowerObsBM.i)<-paste0("V",1:nCU)
      names(counterUpperObsBM.i)<-paste0("V",1:nCU)
      names(totalCatch.i)<-paste0("V",1:nCU)
      names(totalCACatch.i)<-paste0("V",1:nCU)
      names(totalUSCatch.i)<-paste0("V",1:nCU)
    ...
    counterLowerBM.i<-counterLowerBM.i %>% tibble::add_column(year=1:nrow(counterLowerBMArray)) %>%
      tibble::add_column(iteration=rep(i,nrow(counterLowerBMArray)))
    counterUpperBM.i<-counterUpperBM.i %>% tibble::add_column(year=1:nrow(counterUpperBMArray)) %>%
      tibble::add_column(iteration=rep(i,nrow(counterUpperBMArray)))
    counterLowerObsBM.i<-counterLowerObsBM.i %>% tibble::add_column(year=1:nrow(counterLowerObsBMArray)) %>%
      tibble::add_column(iteration=rep(i,nrow(counterLowerObsBMArray)))
    counterUpperObsBM.i<-counterUpperObsBM.i %>% tibble::add_column(year=1:nrow(counterUpperObsBMArray)) %>%
      tibble::add_column(iteration=rep(i,nrow(counterUpperObsBMArray)))
    totalCatch.i<-totalCatch.i %>% tibble::add_column(year=1:nrow(totalCatchArray)) %>%
      tibble::add_column(iteration=rep(i,nrow(totalCatchArray)))
    totalCACatch.i<-totalCACatch.i %>% tibble::add_column(year=1:nrow(totalCACatchArray)) %>%
      tibble::add_column(iteration=rep(i,nrow(totalCACatchArray)))
    totalUSCatch.i<-totalUSCatch.i %>% tibble::add_column(year=1:nrow(totalUSCatchArray)) %>%
      tibble::add_column(iteration=rep(i,nrow(totalUSCatchArray)))

    sMSYEst.i<-sMSYEst.i %>% tibble::add_column(year=1:nrow(estSMSY)) %>%
      tibble::add_column(iteration=rep(i,nrow(estSMSY)))
    sGenEst.i<-sGenEst.i %>% tibble::add_column(year=1:nrow(estSGen)) %>%
    ...
    counterUpperObsBM_long.i <- counterUpperObsBM.i %>%
      tidyr::pivot_longer(tidyr::starts_with("V"),names_to="CU", values_to="aboveUpperObsBM")


    totalCatch_long.i <- totalCatch.i %>%
    ...
    sMSYEst_long.i <- sMSYEst.i %>%
      tidyr::pivot_longer(tidyr::starts_with("V"),names_to="CU", values_to="sMSYEst")
    sGenEst_long.i <- sGenEst.i %>%
      tidyr::pivot_longer(tidyr::starts_with("V"),names_to="CU", values_to="sGenEst")
    ...
    bmUMSY.i <- bmUMSY.i %>%
     ...
     HCRDat_long.i <- counterLowerBM_long.i %>% tibble::add_column(aboveUpperBM=counterUpperBM_long.i$aboveUpperBM) %>%
        tibble::add_column(aboveLowerObsBM=counterLowerObsBM_long.i$aboveLowerObsBM) %>%
        tibble::add_column(aboveUpperObsBM=counterUpperObsBM_long.i$aboveUpperObsBM) %>%
        tibble::add_column(totalCatch=totalCatch_long.i$totalCatch) %>%
        tibble::add_column(totalCACatch=totalCACatch_long.i$totalCACatch) %>%
    ...
    if (i > 1) {
      hcrDatout <- dplyr::bind_rows(hcrDatout,HCRDat_long.i)
    }
  }
